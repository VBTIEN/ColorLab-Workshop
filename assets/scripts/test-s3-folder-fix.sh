...
aws s3 ls "s3://$BUCKET/uploads/" --recursive | head -10
...
aws s3 ls "s3://$BUCKET/uploads/" --recursive | tail -10
...
for i in {1..3}; do
    ...
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}    ✅ Upload $i successful${NC}"
    else
